(function() {
  let colors = ["white", "orange", "green", "yellow", "red", "purple", "blue"];
  ...
  function move(dir) {
    if (!shouldMoveColor) {
      ...
      let color = board[selected[0]][selected[1]];
      next = [selected[0], selected[1]];
      while (inBound(next[0] + dir[0], next[1] + dir[1]) && board[next[0] + dir[0]][next[1] + dir[1]] == 0) {
      ...
      if (!inBound(next[0] + dir[0], next[1] + dir[1])) {
        ...
        document.getElementById(selected[0] + "-" + selected[1]).style.background = colors[0];
        document.getElementById(selected[0] + "-" + selected[1]).classList.remove("selected-for-move");
        board[next[0]][next[1]] = color;
        document.getElementById(next[0] + "-" + next[1]).style.background = colors[color];
        moveSelectedSquareTo(next[0], next[1]);
        detectSuccess();
        syncUndoVisibility();
  ...
  function doUndo() {
    if(gameOver) {
      ...
      let undoElem = undoState.pop();
      let color =  undoElem[0];
      let lastPos = undoElem[1];
      for (i = 0; i < 5; i++) {
        for (j = 0; j < 5; j++) {
          if(board[i][j] == color) {
            board[i][j] = 0;
            ...
            document.getElementById(lastPos[0] + "-" + lastPos[1]).style.background = colors[color];
            break;
          }
        }
      }
  ...
  function calcShouldMoveColor() {
    ...
    if (shouldMoveColor) {
      document.getElementById(selected[0] + "-" + selected[1]).classList.add("selected-for-move");
    } else {
      document.getElementById(selected[0] + "-" + selected[1]).classList.remove("selected-for-move");
    }
  }
